function ePic = update4Robots(ePic,ports,speedsRL)
...
N=length(ports);
if N~=4
    error('numero robot diverso da 4');
end
...
tx(8) = floor(str2double(ports{1})/256);
tx(9) = bitand(str2double(ports{1}), 255);

% Specify address of the second robot.
tx(17) = floor(str2double(ports{2})/256);
tx(18) = bitand(str2double(ports{2}), 255);

% Specify address of the third robot.
tx(26) = floor(str2double(ports{3})/256);
tx(27) = bitand(str2double(ports{3}), 255);

% Specify address of the fourth robot.
tx(35) = floor(str2double(ports{4})/256);
tx(36) = bitand(str2double(ports{4}), 255);
...
tx(1) = 100;%ePic.set.rgb(1);
tx(2) = 100;%ePic.set.rgb(3);
tx(3) = 100;%ePic.set.rgb(2);
