function ePic = updateNRobots(ePic,elisas,speedsRL)
...
for ii=1:4:N
    ...
    idx2=min(ii+4-1,N);
    Ncur=idx2-idx1+1;
    elisasCur=elisas(idx1:idx2);
    speedsRLcur=speedsRL(idx1:idx2,:);
    % tx[1..9] => 1st robot payload
    % tx[10..18] => 2nd robot payload
    % tx[19..27] => 3rd robot payload
    % tx[28..36] => 4th robot payload
    tx = uint8(zeros([1 Ncur*9]));
    for i=1:Ncur
        idx=(i-1)*9+8;
        % Specify address of the first robot (from interface).
        tx(idx) = floor(str2double(elisasCur(i).port)/256);
        tx(idx+1) = bitand(str2double(elisasCur(i).port), 255);
    end
    for i=1:Ncur
        wR=speedsRLcur(i,1);
        wL=speedsRLcur(i,2);
        ...
        %     % right speed
        if(wR >= 0)
            tx(idx) = bitor(typecast(int8(wR),'uint8'), hex2dec('80'));
            %ePic.value.speed(2)=typecast(uint8(tx(5)), 'int8');
        else
            tx(idx) = typecast(-int8(wR),'uint8');
            %ePic.value.speed(2)=typecast(uint8(tx(5)), 'int8');
        end
        
        % left speed
        if(wL >= 0)
            tx(idx+1) = bitor(typecast(int8(wL),'uint8'), hex2dec('80'));
            %ePic.value.speed(1)=typecast(uint8(tx(6)), 'int8');
        else
            tx(idx+1) = typecast(-int8(wL),'uint8');
            %ePic.value.speed(1)=typecast(uint8(tx(6)), 'int8');
        end
    ...
    tx(idx)   = elisasCur(i).ePic.set.rgb(1);
    tx(idx+1) = elisasCur(i).ePic.set.rgb(3);
    tx(idx+2) = elisasCur(i).ePic.set.rgb(2);
    %{
    ...
    if(elisasCur(i).ePic.set.irTx(1)==1)
        tx(idx+3) = bitor(tx(idx+3), bitsll(uint8(1), 0));
    else
        tx(idx+3) = bitand(tx(idx+3), bitcmp(bitsll(uint8(1), 0)));
    end
    if(elisasCur(i).ePic.set.irTx(2)==1)
        tx(idx+3) = bitor(tx(idx+3), bitsll(uint8(1), 1));
    else
        tx(idx+3) = bitand(tx(idx+3), bitcmp(bitsll(uint8(1), 1)));
    end
    %{
