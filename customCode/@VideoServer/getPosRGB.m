function [trajK2,rawK2,collision] = getPosRGB(imgRGB,trajK1,rawK1,nAgents,rawMean,tol)

% trajKx = nAgents x 2
parameters;

thr=ErosionThr;
raw=3;
[BW1,auxRGB1] = VideoServer.imgThresholdErode(imgRGB,thr,raw);

collision=0;

%% find Agents
%[centers,radii1,radii2]=VideoServer.findAgents(auxRGB1);
[centers, radii] = imfindcircles(auxRGB1,[15 25]);
radii1=radii-0.05;
radii2=radii+0.05;

% if sum(radii1)==0
%     error('frame perso!!!')
% end

% number of detected agents
nAd = size(centers,1);


%%% Filter
%--------------------------------------------------------------------
%delete all radius that can't be robots
% indexNotAg=[];
% for i=1:nAd
%     ratRad=radii1(i)/radii2(i);
%     c1=norm(radii1(i)-rawMean(1));
%     c2=norm(radii2(i)-rawMean(2));
%     if ratRad<0.2 || ratRad>2 ||...
%         c1 > tol || c2 > tol
%         indexNotAg=[indexNotAg; i];
%     end
% end
% centers(indexNotAg,:)=[];
% radii1(indexNotAg)=[];
% radii2(indexNotAg)=[];
% nAd=nAd-length(indexNotAg);
%--------------------------------------------------------------------


if isempty(trajK1)
    if nAd==nAgents
        for i=1:nAgents
            trajK2(i,:) = centers(i,:);
            rawK2(i,:)  = [radii1(i,:) radii2(i,:)];
        end
    else
        error('Collisions exist in initial framework!');
    end
else
    if nAd==nAgents
        collision = 0;
        couple=VideoServer.findCouples(trajK1,centers);
        for i=1:nAgents
            trajK2(i,:) = centers(couple(i),:);
            rawK2(i,:)  = [radii1(couple(i),:) radii2(couple(i),:)];
        end
    else
        if nAd<nAgents
            %% collisione
            isAssigned=zeros(1,nAgents);
            [nFA,freeAgents,cAF,r1F,r2F,cAB,r1B,r2B]=VideoServer.findFree(centers,radii1,radii2,rawMean,tol);
            if nFA>0
                couple=VideoServer.findCouples(trajK1,cAF);
                for i=1:nAgents
                    if couple(i)>0
                        trajK2(i,:) = cAF(couple(i),:);
                        rawK2(i,:)  = [r1F(couple(i)) r2F(couple(i))];
                        isAssigned(i)=1;
                    end
                end
            end
            % %             isAssigned
            % resta da assegnare gli altri agenti alle rimanenti componenti
            couple=zeros(nAgents,1);
            D = zeros(nAgents,nAd);
            for i=1:nAgents
                for j=1:nAd
                    D(i,j)=norm(trajK1(i,:)-centers(j,:));
                end
            end
            for i=1:nAgents
                minD=min(D(:));
                [r,c]=find(D==minD);
                r=r(1); c=c(1);
                couple(r) = c;
                D(r,:)=inf;
                if freeAgents(c)
                    D(:,c)=inf;
                end
            end
            for i=1:nAgents
                if couple(i)>0 && ~isAssigned(i)
                    trajK2(i,:) = centers(couple(i),:);
                    rawK2(i,:)  = [radii1(couple(i)) radii2(couple(i))];
                end
            end
        else
            for i=1:nAgents
                trajK2(i,:)=trajK1(i,:);
                rawK2(i,:)=rawK1(i,:);
            end
            disp('Troppe componenti!!!');
        end
    end
end
