%% STATIC: setSpeed4Robots(ports,wRvec,wLvec,isRadS,iswRwL);
%
...
function[]=setSpeed4Robots(ports,wRvec,wLvec,isRadS,iswRwL)
%updateNRobots(ePic,ports,speedsRL)
N=length(ports);
speedsRL=zeros(N,2);
for i=1:N
    wR=wRvec(i);
...
%
elisaTMP=Elisa3(ports{1});
elisaTMP.connect();
elisaTMP.ePic=update4Robots(elisaTMP.ePic,ports,speedsRL);
elisaTMP.disconnect();
end